...
export default function Sidebar({children}: { children: ReactNode }) {
  ...
  return <SidebarContext.Provider value={{projectName:project,sidebar,transform:()=>setSidebar(v=>!v),curMenu: menus.find(menu=>menu.id === Number(curId))}}>
    <div className={cn('flex-shrink-0 border-r  fixed h-full transition-transform overflow-hidden sm:transition-width  sm:relative',sidebar?'translate-x-0 w-[180px]':'-translate-x-full sm:w-0')}>
